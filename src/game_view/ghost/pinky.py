...
class Pinky:
    """Represents the Pinky ghost in the Pac-Man game.

    Pinky chases the player directly by computing the shortest path
    to the player's position using a breadth-first search (BFS) of
    the maze. When the player is in "super" mode (``super_mod``),
    Pinky instead flees from the player, heading toward the
    ...
    def __init__(self, manager: Any) -> None:
        ...
        self.cell_pos: List[int] = [0, self.map.current.height - 1]
        x, y = self.cell_pos
