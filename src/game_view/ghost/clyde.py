...
class Clyde:
    """Represents the Clyde ghost in the Pac-Man game.

    Clyde chases the player directly by computing the shortest path
    to the player's position using a breadth-first search (BFS) of
    the maze. When the player is in "super" mode (``super_mod``),
    Clyde instead flees from the player, heading toward the
    ...
    def __init__(self, manager: Any) -> None:
        ...
        self.cell_pos: List[int] = [self.map.current.width - 1, 0]
        x, y = self.cell_pos
